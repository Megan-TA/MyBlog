...
1. 请求类型 get/post/head
2. 除了常见的请求头外，无额外自定义请求头
...
## 复杂请求
浏览器先发出options预检请求，服务器确认后再发出真实请求

为了减少频繁的options 预检，可以在服务器的预检响应中设置`Access-Control-Max-Age: <秒数>` ，浏览器会把这条`URL+Origin+请求方法+请求头`的组合结硬缓存在内存里，规定时间内不会再发第二次
